...
public interface ChatRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query("SELECT c FROM ChatMessage c WHERE c.usuarioId = :usuarioId AND c.sessionId = :sessionId ORDER BY c.criadoEm DESC LIMIT 20")
    List<ChatMessage> findRecentMessages(Long usuarioId, String sessionId);

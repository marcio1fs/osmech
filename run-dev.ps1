...
# Pré-requisitos: PostgreSQL rodando, Java 21+, Flutter instalado, Maven
#
...
function Start-Frontend {
    ...
    if ($UseChrome) {
        ...
        $cmd = "cd '$frontendDir'; flutter pub get; flutter run -d chrome --web-port 8083"
    } else {
        Write-Host "[OSMECH] Frontend em modo web-server (mais estável)." -ForegroundColor Green
        $cmd = "cd '$frontendDir'; flutter pub get; flutter run -d web-server --web-port 8083"
    }
...
if (-not $BackendOnly -and -not $FrontendOnly) {
    ...
    Start-Frontend
    Write-Host "`n[OSMECH] Backend: http://127.0.0.1:8081" -ForegroundColor Green
    ...
    Start-Backend
    Write-Host "`n[OSMECH] Backend: http://127.0.0.1:8081`n" -ForegroundColor Green
} elseif ($FrontendOnly) {
    Start-Frontend
